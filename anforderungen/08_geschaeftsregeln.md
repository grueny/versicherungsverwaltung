...
| GR-P13 | Der Versicherungsgewerbetarif (VGT) wird als prozentualer Nachlass auf Vertragsebene (Nachlassart `VGT_NACHLASS`) abgebildet. Die Nachlasshöhe ist pro Sparte konfigurierbar (Standard: 15 %). Pro Angebot/Antrag/Vertragsstand ist maximal ein VGT-Nachlass zulässig | Automatisierter Branchennachlass für Mitarbeitende der Versicherungswirtschaft | Pro `referenz_typ` + `referenz_id` darf maximal ein Eintrag mit `nachlassart = 'VGT_NACHLASS'` existieren; Nachlasshöhe aus Spartenkonfiguration | UC-11 |
| GR-P14 | Die VGT-Berechtigung wird am Partner gespeichert (Arbeitgeber, Beschäftigungsverhältnis, Nachweis-Referenz, Gültigkeitszeitraum) und jährlich zum Hauptfälligkeitstermin automatisch geprüft. Bei Ablauf wird der VGT-Nachlass entfernt und eine Wiedervorlage erzeugt | Jährliche Berechtigungsprüfung sichert korrekte Nachlassvergabe | VGT-Berechtigung am Partner prüfen: `vgt_gueltig_ab ≤ Vertragsbeginn` und `vgt_gueltig_bis IS NULL OR ≥ nächster Versicherungsbeginn` | UC-11 |
...
| GR-A13 | Bei der Angebotserstellung für einen VGT-berechtigten Partner wird der VGT-Nachlass automatisch vergeben, sofern die Berechtigung zum Vertragsbeginn gültig ist und die Nachlasshöhe für die Sparte konfiguriert ist. Der Benutzer wird per Hinweis informiert | Automatisierte Nachlassvergabe für Mitarbeitende der Versicherungswirtschaft | VGT-Berechtigung am Partner vorhanden und gültig; VGT-Nachlasssatz für Sparte konfiguriert | UC-11 |
